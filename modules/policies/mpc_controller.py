from modules.policies.base import Policy
...
class MPCController(Policy, Serializable):
    ...
    def get_actions(self, observations, env_infos):
        env_infos = np.array([env_info['info'] for env_info in env_infos])
        if self.use_cem:
            actions = self.get_cem_action(observations, env_infos)
        else:
            actions = self.get_rs_action(observations, env_infos)
    ...
    def get_rs_action(self, observations, env_infos):
        n = self.n_candidates
        m = len(observations)
        h = self.horizon
        returns = np.zeros((n * m,))
        ...
        for t in range(h):
            if t == 0:
                cand_a = a[t].reshape((m, n, -1))
                observation = np.repeat(observations, n, axis=0)
                env_info = np.repeat(env_infos, n, axis=0)
            next_observation = self.dynamics_model.predict(observation, a[t], env_info)
