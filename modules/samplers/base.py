...
class SampleProcessor(object):
    ...
    def _log_path_stats(self, paths, log=False, log_prefix='', tb_extra=None):
        # compute log stats
        ...
        # write to tb
        itr = tb_extra[0]
        tb_writer = tb_extra[1]
        with tb_writer.as_default():
            tf.summary.scalar('itr/AverageDiscountedReturn', average_discounted_return, step=itr)
            tf.summary.scalar('itr/AverageReturn', np.mean(undiscounted_returns), step=itr)
            tf.summary.scalar('itr/StdReturn', np.std(undiscounted_returns), step=itr)
            tf.summary.scalar('itr/MaxReturn', np.max(undiscounted_returns), step=itr)
            tf.summary.scalar('itr/MinReturn', np.min(undiscounted_returns), step=itr)
